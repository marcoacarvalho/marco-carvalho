...
def mainloop (win,msg_text):
    while 1:
        ...
        if ans == 'l':
            ...
            sure(win,"Tem certeza (y/n)")
    return 0

def ui(win,title="Test"):
    times = 0
    app_string = string.center("Registro de MAC Address no DHCP",79)
    cent = 0
    app_string = app_string[:79]
    win.addstr(0,cent,app_string,curses.A_STANDOUT)
    win.addstr(1,0,string.center(title,79),curses.A_BOLD)
    win.addstr(7,4,"Main window text")

    while 1:
        win.refresh()

        if mainloop(win,"Escolha uma opcao"):
            break
...
if __name__=='__main__':
    ...
    try:        # Initialize curses
        ...
        stdscr.keypad(1)
        ui(stdscr,title=string.center("l=listar | n=novo | a=alterar | e=excluir | s=sair",79))         # Enter the mainloop
        stdscr.keypad(0)
